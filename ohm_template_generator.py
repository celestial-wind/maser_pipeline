# ohm_template_generator.py (Regenerated Version)
...
The primary user-facing function for OHM signals is `generate_final_template()`.
"""
...
def apply_pfb_smoothing(spectrum: np.ndarray, kernel_width: int) -> np.ndarray:
    ...
    by convolving the spectrum with a Hamming window.
    ...
    kernel = windows.hamming(kernel_width)
...
def generate_boxcar_template(width: int) -> np.ndarray:
    """Generates a simple boxcar (top-hat) template of a given width."""
...
def process_to_native_resolution(
...
def generate_final_template(
    ...
    """
    Generates a final, instrument-ready matched-filter template for a given redshift.

    This is the primary high-level function for creating a realistic OHM template. 
    It encapsulates the full process of creating an intrinsic average template 
    and then processing it for a specific observational setup.

    Args:
        z: The target redshift.
        vel_axis_kms: High-res velocity axis (e.g., np.linspace(-1200, 1200, 4096)).
        native_freq_grid: The array of center frequencies of the telescope.
        N_population: Number of synthetic masers to average for the intrinsic template.

    Returns:
        A tuple containing:
        - The final, native-resolution matched-filter template.
        - The start index in the full native grid.
        - The end index in the full native grid.
    """
    # Step 1: Create the redshift-independent intrinsic template
    intrinsic_template = create_intrinsic_template(N_population, vel_axis_kms)
    
    # Step 2: Process it for the specific redshift and instrument
    final_template, start_idx, end_idx = process_to_native_resolution(
        intrinsic_template_v=intrinsic_template,
...
if __name__ == '__main__':
    ...
    print("--- Running OHM Template Generator Demonstration ---")
    
    # --- Demo 1: Generate a realistic OHM template ---
    print("\n1. Generating a realistic OHM template for z=2.0...")
    z_target = 2.0
    vel_axis = np.linspace(-1200, 1200, 4096)
    
    # Note: Using a smaller N_population for a quick demonstration.
    final_template, start_idx, end_idx = generate_final_template(
        z=z_target,
        vel_axis_kms=vel_axis,
        native_freq_grid=CHIME_FREQS,
        N_population=1000  # For a real search, use N > 5000
    )

    if final_template.size > 0:
        freq_axis_subset = CHIME_FREQS[start_idx:end_idx]
        print(f"  - Success! Template has {len(freq_axis_subset)} channels.")
        
        fig, ax = plt.subplots(figsize=(10, 6))
        ax.plot(freq_axis_subset, final_template, 'o-', label=f'Realistic OHM (z={z_target})')
        ax.set_title('Template Generator Demonstration')
        ax.set_xlabel('Frequency (MHz)')
        ax.set_ylabel('Normalized Amplitude')
    else:
        print("  - Could not generate template. Redshift may be out of band.")

    # --- Demo 2: Generate simple templates ---
    print("\n2. Generating simple baseline templates...")
    template_width = 20 # Example width of 20 channels
    box_template = generate_boxcar_template(template_width)
    gauss_template = generate_gaussian_template(template_width)
    
    # Plot simple templates on the same axis for comparison
    # We'll just use the first 20 channels of the frequency axis for plotting
    freq_axis_simple = CHIME_FREQS[:template_width]
    if 'ax' in locals():
        ax.step(freq_axis_simple, box_template * 0.5, where='mid', label=f'Boxcar (width={template_width})')
        ax.plot(freq_axis_simple, gauss_template * 0.8, 's--', label=f'Gaussian (width={template_width})')
        ax.legend()
        ax.grid(True)
        plt.show()